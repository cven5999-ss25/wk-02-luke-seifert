...
The mutate function is first used to create the variable "capita_kg_year." Then it is used to change "income_id" to a new version of itself.
...
The first mutate divided waste per year in tons by corresponding populations and changed tons to kg, so 'capita_kg_year' shows how much kg of waste each person is generating in a year. The second mutate reorganized the data from continents into income classes.

Describe in your own words what the 'color' argument inside the `aes()` does

'color' made the different income levels their own color, making the plots much more visually appealing.

6.  Describe in your own words what the `labs()` function does

'labs' customizes the graph information to fit the data. In this context it creates a proportional y-axis scale and the legend on the side.
